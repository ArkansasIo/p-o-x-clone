...
bool CreatureDB::load_from_sram(const uint8_t* sram, size_t len) {
    if (len < slots_.size() * 20) return false;
    for (size_t i = 0; i < slots_.size(); ++i) {
        std::array<uint8_t, 20> rec{};
        std::memcpy(rec.data(), sram + i * 20, 20);
...
void CreatureDB::save_to_sram(uint8_t* sram, size_t len) const {
    if (len < slots_.size() * 20) return;
    for (size_t i = 0; i < slots_.size(); ++i) {
        auto rec = slots_[i].serialize();
        std::memcpy(sram + i * 20, rec.data(), 20);
    }
}
